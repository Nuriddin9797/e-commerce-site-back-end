...
from functions.users_function import get_users, create_user_f, update_user_f, delete_user_f, create_general_user_f
...
def create_general_user(form: CreateGeneralUser, db: Session = Depends(database),
                current_user: CreateUser = Depends(get_current_active_user)):
    create_general_user_f(form, db, current_user)
    raise HTTPException(status_code=200, detail="Amaliyot muvaffaqiyatli amalga oshirildi")
